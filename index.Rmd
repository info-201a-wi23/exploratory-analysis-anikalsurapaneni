...
From our analysis, we found that overall there are `sites_in_danger` total heritage sites in danger in the world. Furthermore, the region with the most sites in danger was `region_with_most_sites_in_danger` with `region_num_in_danger`. However, further analyzing the data, we actually found that the country with the most sites in danger was actually outside that region which was `country_with_most_sites_in_danger` with `country_num_in_danger`. 
...
The reason why the table only displays 8 countries is because the rest of the countries only had one site in danger. Because of this, we can 
